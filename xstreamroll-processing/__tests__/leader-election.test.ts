...
jest.mock("pg", () => {  const calls: { sql: string; params: unknown[] }[] = []  
...
describe("PostgresLockManager (SQL contract)", () => {
  ...
  // ─── Additional edge-case coverage (#401) ────────────────────────────────

  it("renew returns false when the same row is now owned by a different worker (takeover)", async () => {
    pgMock.__test.reset()
    const mgr = new PostgresLockManager({
      workerId: "w1",
      databaseUrl: "postgres://x",
    ...
    // Simulate the row being claimed by another worker in the meantime
    // — the UPDATE matches no rows because owner_id != "w1".
    pgMock.__test.setNextResponse({ rows: [], rowCount: 0 }
)
    expect(await mgr.renew("s1", token!)).toBe(false)

    // The natural next action is for the worker to drop its session:
    // release also matches 0 rows and returns false cleanly.
    expect(await mgr.release("s1", token!)).toBe(false)
    await mgr.close()
  }
)

  it("release is no-op when called for a stream that was never claimed", async () => {
    pgMock.__test.reset()
    const mgr = new PostgresLockManager({
      workerId: "w1",
      databaseUrl: "postgres://x",
    ...
    expect(await mgr.release("unknown", synthetic)).toBe(false)
    await mgr.close()
  }
)

  it("releaseAll only removes locks owned by this worker", async () => {
    pgMock.__test.reset()
    const mgr = new PostgresLockManager({
      workerId: "w1",
      databaseUrl: "postgres://x",
    ...
    await mgr.install()
    pgMock.__test.calls.length = 0
    pgMock.__test.setNextResponse({ rows: [], rowCount: 4 }
)
    await mgr.releaseAll()
    const sql = pgMock.__test.calls[0]?.sql
    expect(sql).toMatch(/DELETE FROM stream_locks WHERE owner_id = \$1/)
    // The bind parameter for the WHERE clause must be this worker's
    // id, not a wildcard.
    expect(pgMock.__test.calls[0]?.params[0]).toBe("w1")
    await mgr.close()
  }
)

  it("acquire that returns a row owned by a different worker is treated as lost race", async () => {
    pgMock.__test.reset()
    pgMock.__test.setNextResponse({
      rows: [
        {
          owner_id: "someone-else",
          owner_token: "irrelevant",
...
,
  ...
  // The "concurrent acquire" scenario is covered indirectly by the
  // takeover-renew and foreign-owner cases above, plus the in-memory
  // suite's cross-worker refusal. Adding a third race-window mock here
  // made the file brittle: any test that re-patched
  // `pgMock.__test.setNextResponse` raced with sibling tests in
  // `jest --runInBand`. Keeping this coverage in the SQL contract suite
  // was deemed too risky for the surface area gained.
}
)
