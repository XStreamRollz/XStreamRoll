  ...
  describe("health endpoints", () => {
    ...
    it("GET /healthz returns 503 when shutting down", async () => {
      ...
      try {
        ...
        expect((err as { response: { status: number; data: { status: string } } }).response.status).toBe(503)
        expect((err as { response: { data: { status: string } } }).response.data.status).toBe("shutting-down")
      }
    })
  })
})
